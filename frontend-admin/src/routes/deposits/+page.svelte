<script lang="ts">
  ...
  async function processTx(id: string, action: string) {
    ...
    try {
      ...
      toasts.add(action === 'approve' ? 'تمت الموافقة على الإيداع' : 'تم رفض الإيداع', 'success');
    } catch (e: unknown) {
...
<div class="animate-fade-in">
  <div class="flex items-center justify-between mb-6">
    <h1 class="text-2xl font-bold">الإيداعات المعلقة</h1>
  ...
  {:else if deposits.length === 0}
    <div class="card text-center py-10">
      ...
      <p class="text-gray-500">لا توجد إيداعات معلقة</p>
    </div>
  {:else}
        ...
        <tbody>
          {#each deposits as deposit, i}
            <tr class="border-b border-dark-700 hover:bg-dark-700/50 transition-colors">
              <td class="text-xs text-gray-400">{formatDate(deposit.created_at)}</td>
              <td class="font-medium">{deposit.user_email}</td>
              <td class="font-mono">{formatAmount(deposit.amount)}</td>
              <td><span class="badge badge-active">{deposit.currency}</span></td>
              <td>
                <div class="flex gap-2">
                  <button
                    class="btn-success btn-xs"
                    disabled={processing === deposit.tx_id}
                    on:click={() => processTx(deposit.tx_id, 'approve')}
                  >
                    {processing === deposit.tx_id ? '...' : 'موافقة'}
                  </button>
                  <button
                    class="btn-danger btn-xs"
                    disabled={processing === deposit.tx_id}
                    on:click={() => processTx(deposit.tx_id, 'reject')}
                  >
                    {processing === deposit.tx_id ? '...' : 'رفض'}
                  </button>
                </div>
              </td>
            </tr>
